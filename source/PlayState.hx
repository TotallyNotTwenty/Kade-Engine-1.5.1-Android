...
//here goes mayhem babe~ lmaoooooo
	var bgLights:FlxTypedGroup<FlxSprite>;
  ...
  var max:FlxSprite;
    ...
    case 'festivalDay':
			{
				curStage = 'festivalDay';
        defaultCamZoom = 0.64;
				var bg = new FlxSprite(-550, -160).loadGraphic(Paths.image('mayhem/morning/bg', 'shared'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);

        var stage:FlxSprite = new FlxSprite(-510, -260).loadGraphic(Paths.image('mayhem/stage', 'shared'));
				stage.antialiasing = true;
				stage.active = false;
				add(stage);

        bgLights = new FlxTypedGroup<FlxSprite>();
				add(bgLights);

				for (i in 0...4)
				{
					var light:FlxSprite = new FlxSprite(-510, -260).loadGraphic(Paths.image('mayhem/light' + i, 'shared'));
					if (i != 0)
						light.visible = false;
					light.antialiasing = true;
					bgLights.add(light);
        ...
        headlights = new FlxSprite(-510, -80);
				headlights.frames = Paths.getSparrowAtlas('mayhem/headlights', 'shared');
				headlights.antialiasing = true;
				headlights.animation.addByPrefix('idle', 'lightsrepeated', 24, false);
				headlights.animation.play('idle');
				add(headlights);

				max = new FlxSprite(1100, 225);
        max.frames = Paths.getSparrowAtlas('mayhem/Max', 'shared');
        max.animation.addByPrefix('idle', 'MAXIDLE', 24, false);
        max.animation.addByPrefix('singUP', 'MAXUP', 24, false);
        max.animation.addByPrefix('singDOWN', 'MAXDOWN', 24, false);
        max.animation.addByPrefix('singLEFT', 'MAXLEFT', 24, false);
        max.animation.addByPrefix('singRIGHT', 'MAXRIGHT', 24, false);
        max.animation.play('idle');
 				add(max);

				abel = new FlxSprite(-180, 140);
        abel.frames = Paths.getSparrowAtlas('mayhem/Abel', 'shared');
        abel.animation.addByPrefix('idle', 'ABLENOGUITAR', 24, false);
        abel.animation.addByPrefix('singUP', 'ABELUP', 24, false);
        abel.animation.addByPrefix('singDOWN', 'ABELDOWN', 24, false);
        abel.animation.addByPrefix('singLEFT', 'ABELLEFT', 24, false);
        abel.animation.addByPrefix('singRIGHT', 'ABELRIGHT', 24, false);
        abel.animation.play('idle');
 				add(abel);

        frontbop = new FlxSprite(-510, 950);
				frontbop.frames = Paths.getSparrowAtlas('mayhem/frontboppers', 'shared');
   ...
   case 'festivalLate':
			{
				curStage = 'festivalLate';
        defaultCamZoom = 0.64;
				var bg = new FlxSprite(-550, -160).loadGraphic(Paths.image('mayhem/dusk/bg', 'shared'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.5, 0.5);
				bg.active = false;
				add(bg);
	...
	private function generateStaticArrows(player:Int):Void
	{
			...
			if (!isStoryMode)
		{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
				tween = FlxTween.tween(babyArrow, {y: babyArrow.y + 10, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
/*
...
/*
					case 'cj':
						camFollow.y = dad.getMidpoint().y + 395.97;
						camFollow.x = dad.getMidpoint().x + 100;
*/
				}
...
/*
    if (curSong == "inverted-ascension")
  {
    switch (curBeat)
    {
    case 1, 2, 3, 4:
    abel.animation.play('singUP', true);
    case 5, 6, 7, 8:
    abel.animation.play('singDOWN', true);
    case 9, 10, 11, 12:
    abel.animation.play('singRIGHT', true);
    case 13, 14, 15, 16:
    abel.animation.play('singLEFT', true);
    case 17, 18, 19, 20:
    abel.animation.play('singUP', true);
    case 21, 22, 23, 24:
    abel.animation.play('singDOWN', true);
...
*/
		...
		switch (curStage)
		{
			...
			case "festivalDay" | "festivalLate":

				if (curBeat % 4 == 0)
				{
					bgLights.forEach(function(light:FlxSprite)
					{
						light.visible = false;
					});

					curLight = FlxG.random.int(0, bgLights.length - 1);

					bgLights.members[curLight].visible = true;
					// phillyCityLights.members[curLight].alpha = 1;

           abel.animation.play('idle', true);

           max.animation.play('idle', true);
